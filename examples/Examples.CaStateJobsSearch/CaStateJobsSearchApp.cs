...
namespace Simple.Selenium.Examples.CaStateJobsSearch
{
...
";
        ...
        internal CaStateJobsSearchApp(BrowserType browserType)
            : base(browserType, "https://jobs.ca.gov/")
        {
        }
    ...
    class JobSearchPage : BasePage<JobSearchPage>
    {
        ...
        {
            jobTitleTextField.EnterText(jobTitle);
            if (titleSearchOnly)
            {
    ...
    class JobVacancySearchResultsPage : BasePage<JobVacancySearchResultsPage>
    {
        ...
        public void LoopThroughJobVacancies(SimpleHtmlWriter htmlWriter)
        {
            ...
            IReadOnlyCollection<IWebElement> resultPages = this.table.GetWebElement().FindElements(By.CssSelector("tfoot table td"));
        ...
        public virtual void GotoPage(int pageIdx)
        {
            ...
            try
            {
                IWebElement pageLinkElement = this.table.GetWebElement().FindElement(By.CssSelector("tfoot table tbody tr td:nth-child(" + (pageIdx + 1) + ") a"));
    ...
    class JobPostPage : BasePage<JobPostPage>
    {
        ...
        {
            var jobCtrlNumberText = new TextControl(By.Id("lblDetailsJobControlNumber"));
            string jobCtrlNumber = jobCtrlNumberText.Text;
            var workingTitleText = new TextControl(By.Id("lblWorkingTitle"));
            if (workingTitleText.ExistsNow())
            {
    ...
    class JobDescriptionSection : Control
    {
        public JobDescriptionSection(By locator, IWebDriver webDriver = null) : base(locator, webDriver)
        {
        }
        ...
        public IList<string> Texts
        {
            get
            {
                IWebElement webElement = this.GetWebElement();
